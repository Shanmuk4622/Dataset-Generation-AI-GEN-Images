% ============================================================
%  SECTION 4 — DATASET CONSTRUCTION (METHODOLOGY)
%  Paste after \section{Related Work} in your main .tex file.
%  New citations used here (add to bibliography):
%    deng2009imagenet, lin2014coco, he2016resnet, dosovitskiy2021vit,
%    tan2019efficientnet, radford2021clip
%  All generator citations already in related_work_and_references.tex
% ============================================================

% ── Required additions to preamble ───────────────────────────────────────────
% \usepackage{booktabs}   % already required by related work
% \usepackage{amsmath}
% \usepackage{amssymb}
...
\textbf{\datasetname} (\texttt{Shanmuk4622/ai-detection-dataset-v2}).
...
\texttt{Salesforce/blip2-opt-2.7b} checkpoint, running beam search
...
\texttt{openai/clip-vit-large-patch14} tokeniser~\cite{radford2021clip}
...
\toprule
Key & Model & Architecture & Native (px) & Steps & Guidance & Reference \\
\midrule
\texttt{sd15}
...
seed computed as:
\begin{equation}
s(m, r) = \left(\mathrm{int}(\mathrm{SHA\text{-}256}
           (\texttt{MASTER\_SEED} : m : r)_{[0:15]}, 16)\right)
           \bmod (2^{31} - 1)
\label{eq:seed}
\end{equation}
where $m$ is the generator key, $r$ is the \texttt{source\_real\_id},
and the first 15 hexadecimal digits of the SHA-256 digest are
...
(\texttt{stabilityai/warp-ai/wuerstchen-prior}) ran for 20 steps with
guidance~$= 4.0$ to produce image embeddings, followed by the decoder
(\texttt{stabilityai/warp-ai/wuerstchen}) for 10 steps with
guidance~$= 0.0$.
...
\State $I \leftarrow \textsc{JpegDecode}(\textsc{JpegEncode}(I, Q, \text{subsampling}=0))$
  \Comment{JPEG equalisation}
...
\texttt{prompt}            & \texttt{string}   & BLIP-2 caption used; \texttt{null} for real rows \\
...
f(r) = \frac{\mathrm{int}(\mathrm{SHA\text{-}256}
        (\texttt{MASTER\_SEED} : r)_{[0:8]}, 16)}{100{,}000}
\bmod 1
\label{eq:split}
\end{equation}
Pairs with $f(r) < 0.70$ are assigned to \emph{train}, those with
...
parallelised across multiple Kaggle accounts using a hash-based
worker-partitioning scheme to avoid shard collisions.
The total generation time was approximately 180 GPU-hours on single
NVIDIA Tesla T4 instances.
...
Repository                    & \texttt{Shanmuk4622/ai-detection-dataset-v2} \\
