...
Section~\ref{sec:experiments} characterise the intrinsic forensic
...
reproduced in Algorithm~\ref{alg:preprocess}) is deterministic and
...
\texttt{results/baseline\_experiments/checkpoints/}\\
in the same HuggingFace repository.
Per-epoch training histories are stored as JSON files
(\texttt{resnet50\_progress.json}, \texttt{efficientnet\_b4\_progress.json},
\texttt{vit\_b16\_progress.json}) alongside the final
